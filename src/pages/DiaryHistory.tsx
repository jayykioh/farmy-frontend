...
export const DiaryHistory: React.FC = () => {
  ...
  const getActivityIcon = (activityType: string) => {
    ...
    if (type.includes('tưới')) return '💧';
    if (type.includes('phân') || type.includes('dinh dưỡng')) return '🍃';
    if (type.includes('thuốc') || type.includes('sâu')) return '🛡️';
    return '🌱';
  };
            ...
            <section className="flex flex-col gap-4 mt-2">
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className="flex-1 bg-primary text-white py-4 rounded-full font-bold text-lg mt-4 shadow-md hover:bg-primary-dark active:scale-95 transition-all cursor-pointer text-center"
                >
                  Thêm hoạt động mới
                </button>
                <button 
                  onClick={() => navigate('/diary')}
                  className="flex-1 bg-white border border-border-main/50 text-text-main py-4 rounded-full font-bold text-lg mt-4 shadow-sm hover:bg-bg-surface active:scale-95 transition-all cursor-pointer"
                >
                  Quay lại danh sách
                </button>
              </div>
