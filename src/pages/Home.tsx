...
export const Home: React.FC = () => {
  ...
  const snaps = snapFeedData?.snaps || [];

  // Fetch authoritative pet status from backend
  const { data: petStatusRaw, refetch: refetchPetStatus } = usePetStatus(user?.id);
      ...
      <div className="flex flex-col md:grid md:grid-cols-12 gap-8 lg:gap-12 w-full">
        ...
        <div className="flex flex-col col-span-12 md:col-span-7 lg:col-span-8 gap-6 md:gap-8">
          ...
          <section className="flex flex-col items-center md:items-start text-center md:text-left mt-2 md:mt-0">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-800 tracking-tight leading-tight">Good morning,<br className="hidden md:block lg:hidden" /> {(user as any)?.displayName || user?.email?.split('@')[0] || 'Nông Dân'}!</h2>
