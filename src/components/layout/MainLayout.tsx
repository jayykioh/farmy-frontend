  ...
  return (
    <div className="flex min-h-[100svh] w-full bg-bg-main font-sans relative">
      ...
      <div className={`flex flex-col flex-1 w-full relative transition-all duration-300 ${showBottomNav ? 'md:ml-[260px]' : ''}`}>
        ...
        {showBottomNav && !isPageHeaderVisible ? (<header className="sticky top-0 w-full z-40 bg-bg-main/90 backdrop-blur-xl flex justify-between items-center px-4 md:px-6 py-2.5 h-14 border-b-2 border-border-main transition-all duration-300">
          <button className="w-10 h-10 flex items-center justify-center text-text-h hover:bg-bg-surface-1 rounded-full active:scale-95 transition-all md:hidden">
          ...
          <h1 className="text-lg font-black text-text-h tracking-tight md:hidden">FARMY AI</h1>
          <div className="hidden md:flex flex-1 items-center max-w-xl relative">
             <svg className="w-5 h-5 absolute left-3 text-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
             </svg>
             <input type="text" placeholder="Tìm kiếm..." className="w-full bg-white border-2 border-border-main rounded-full py-2 pl-10 pr-4 text-xs font-bold text-text-main focus:outline-none focus:border-[#008A5E] transition-all" />
          </div>
          <Link to="/reminders" className="relative w-10 h-10 flex items-center justify-center text-text-h hover:bg-bg-surface-1 rounded-full active:scale-95 transition-all">
