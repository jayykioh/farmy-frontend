...
### text-embedding-004 · Chunk Strategy · BullMQ Async Trigger · pgvector
...
`EmbeddingModule` là module trung tâm chịu trách nhiệm **chuyển đổi văn bản thành vector 768 chiều** bằng Google `text-embedding-004` và lưu vào bảng `memory_embeddings` (PostgreSQL + pgvector). Module này là nền tảng cho toàn bộ tính năng RAG của hệ thống.
...
```typescript
// DiaryService.create() → sau khi INSERT thành công vào PostgreSQL:
