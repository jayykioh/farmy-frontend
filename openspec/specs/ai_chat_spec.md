...
> **MongoDB-first update:** RAG retrieval should use MongoDB Atlas Vector Search and the `knowledge_chunks` / `memory_embeddings` collections described in `mongodb_stack_analysis.md`. Any remaining pgvector/Postgres wording in this file is legacy language to be adapted during backend implementation.
...
```
  ...
  [3. MongoDB Atlas Vector Search] <---------+
         |
         +--> Query A: Vector search `memory_embeddings`
         |             filter: { userId, isActive: true }
         |             topK: 3
         |
         +--> Query B: Vector search `knowledge_chunks`
         |             filter: { cropType?, region?, qualityScore }
         |             topK: 3
         |
...
- [ ] **TC-CHAT-03:** Gửi câu hỏi về bệnh cây trồng có sẵn tài liệu trong MongoDB `knowledge_chunks` -> Kết quả: Kiểm tra logs verify truy vấn Atlas Vector Search được thực thi và kết quả trả ra khớp với thông tin tài liệu bổ trợ.
