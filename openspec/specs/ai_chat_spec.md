...
```
  ...
  [3. pgvector Cosine Search] <--------------+
         |
         +--> Query A: SELECT chunk_text, metadata FROM memory_embeddings
         |             WHERE source_id IN (
         |               SELECT id FROM diary_entries WHERE user_id = $userId
         |             )
         |               AND is_active = TRUE  -- Bắt buộc: bỏ qua embedding đã bị deactivate
         |             ORDER BY embedding <=> $vector LIMIT 3;
         |
         +--> Query B: SELECT chunk_text FROM knowledge_docs
         |             ORDER BY embedding <=> $vector LIMIT 3;
         |
...
- [ ] **TC-CHAT-07:** Tích hợp Pet State vào Prompt -> Đảm bảo khi bắt đầu hoặc tiếp tục trò chuyện, prompt gửi sang Gemini Flash được tự động làm giàu bằng `streak_count` và `pet_mood` chính xác từ Postgres.
