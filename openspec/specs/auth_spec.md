...
*   **Mô tả:** Redirect người dùng đến trang xin quyền của Zalo Official Account với cấu hình `state` chứa thông tin mã hóa của `userId`.
*   **Callback Endpoint:** `GET /api/v1/auth/zalo/callback`
    *   *Query params:* `code`, `state`
    *   *Mô tả:* Nhận authorization code từ Zalo, trao đổi lấy `zalo_user_id` và `access_token`, cập nhật vào thực thể [User](file:///d:/coding/farmdiary/project/backend/src/modules/users/entities/user.entity.ts).
...
| **419 Time Limit** | `AUTH_TOKEN_EXPIRED` | Access Token hết hạn | Gửi request `/auth/refresh` lấy token mới |
...
- [ ] **TC-AUTH-06:** Gọi API với Access Token hết hạn -> Kết quả: 419 / 401.
